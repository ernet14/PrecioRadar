...
- ~~**Pasada de seguridad sobre server actions**~~ — HECHO 2026-05-29. Los 19 actions validan con
  zod y exigen `getCurrentUser`/`requireAdmin` + ownership por `user.id`. Sin cambios de código (ver
  decisiones-recientes). Deuda menor: `id`/`status` sin zod en `promos/bot`, `importar`, `reportes`
  (admin-only, bajo riesgo).
