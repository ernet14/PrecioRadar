...
import {
  GET,
  POST,
} from "../src/app/api/internal/refresh-prices/route";
...
function createRequest(headers?: HeadersInit) {
  return new Request("http://localhost/api/internal/refresh-prices", {
    headers,
  });
}
...
test("rejects refresh prices when CRON_SECRET is missing", async () => {
...
test("rejects refresh prices when the secret does not match", async () => {
...
test("accepts Vercel Cron bearer authorization", async () => {
  process.env.CRON_SECRET = "test-secret";
  clearDatabaseEnv();

  const response = await GET(
    createRequest({ authorization: "Bearer test-secret" }),
  );
  const body = await response.json();
...
test("accepts manual x-cron-secret authorization", async () => {
  process.env.CRON_SECRET = "test-secret";
  clearDatabaseEnv();
